...
function CheckoutContent() {
  ...
  useEffect(() => {
    ...
    // Fetch plan and checkout settings in parallel
    Promise.all([
      fetch("/api/plans").then(r=>r.json()),
      fetch("/api/settings").then(r=>r.json()),
    ]).then(([plansData, settingsData]) => {
      const found = (plansData.plans||[]).find(p => p.id === planId);
      setPlan(found || (plansData.plans||[])[2] || null);
      if (settingsData.checkout) {
        setSettings(settingsData.checkout);
        // Set default crypto to first enabled wallet
        const firstEnabled = Object.entries(settingsData.checkout.wallets||{}).find(([,w])=>w.enabled!==false);
        if (firstEnabled) setCrypto(firstEnabled[0]);
      }
    }).catch(() => {
      const firstEnabled = Object.entries(DEFAULT_SETTINGS.wallets).find(([,w])=>w.enabled!==false);
      if (firstEnabled) setCrypto(firstEnabled[0]);
    });
  }, [planId]);

  const enabledWallets = Object.fromEntries(
    Object.entries(settings.wallets||{}).filter(([,w]) => w.enabled !== false)
  );
  const wallet = enabledWallets[crypto] || Object.values(enabledWallets)[0];
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    setError("");
    if (settings.txidRequired && !txid.trim())   { setError("Please enter your Transaction ID (TXID)."); return; }
    if (!sender.trim())                           { setError("Please enter your sender wallet address."); return; }
    if (settings.screenshotRequired && !fileB64)  { setError("Please upload a payment screenshot."); return; }
    setLoading(true);
    try {
          ...
          <div className="checkout-section">
            <div className="checkout-success">
              ...
              <div className="checkout-success-sub">
                Your payment proof for the <strong>{plan.name} Plan</strong> has been submitted.
                Our team will review and activate your plan within <strong>{settings.activationTime}</strong>.
              </div>
      ...
      <div className="checkout-page">
        <div style={{maxWidth:960,margin:"0 auto"}}>
          <div style={{marginBottom:32}}>
            ...
            <p style={{fontSize:14,color:"var(--text-dim)"}}>{settings.instructions}</p>
          </div>
          ...
          <div className="checkout-wrap">
            ...
            <div className="checkout-left">
              ...
              <div className="checkout-section">
                ...
                <div className="crypto-grid">
                  {Object.entries(enabledWallets).map(([name, info]) => (
                    <div key={name} className={`crypto-card${crypto===name?" selected":""}`} onClick={()=>setCrypto(name)}>
                ...
                {wallet && (
                  <>
                    ...
                    <div className="checkout-warning" style={{marginTop:14}}>
                      ⚠ Send <strong>exactly ${plan.price.toLocaleString()} USD</strong> worth of {wallet.symbol}. {settings.warningText}
                    </div>
                  </>
                )}
              </div>
              ...
              <div className="checkout-section">
                ...
                <form onSubmit={handleSubmit} style={{display:"flex",flexDirection:"column",gap:18}}>
                  {settings.txidRequired !== false && (
                    <div className="form-group">
                      <label className="input-label">Transaction ID (TXID) *</label>
                      <input className="input" placeholder="Enter the transaction hash from your wallet" value={txid} onChange={e=>setTxid(e.target.value)} required />
                      <div style={{fontSize:11,color:"var(--text-muted)",marginTop:4}}>Find this in your wallet's transaction history after sending payment.</div>
                    </div>
                  )}
                  <div className="form-group">
                    <label className="input-label">Your Sender Wallet Address *</label>
                    <input className="input" placeholder="The wallet address you sent payment FROM" value={sender} onChange={e=>setSender(e.target.value)} required />
                  </div>
                  <div className="form-group">
                    <label className="input-label">Payment Screenshot {settings.screenshotRequired ? "*" : "(optional)"}</label>
                    <div className={`upload-area${fileB64?" has-file":""}`} onClick={()=>fileRef.current?.click()}>
            ...
            <div className="checkout-right">
              <div className="plan-summary">
                ...
                <div className="plan-summary-features">
                  {(plan.features||[]).slice(0,5).map((f,i) => (
                    <div key={i} className="plan-summary-feature"><span>✓</span><span>{f}</span></div>
                  ))}
                </div>
              </div>
              ...
              <div className="checkout-section" style={{padding:20}}>
                ...
                {[
                  { label:"Plan",           value:plan.name },
                  { label:"Monthly ROI",    value:`${plan.roi}%`, color:plan.color },
                  { label:"Min Deposit",    value:`$${(plan.minDeposit||0).toLocaleString()}` },
                  { label:"Payment Method", value:wallet?.symbol || "-" },
                ].map((r,i) => (
                  <div key={i} className="order-row">
                    <span className="order-row-label">{r.label}</span>
                    <span className="order-row-value" style={{color:r.color||"var(--text)"}}>{r.value}</span>
                  </div>
                ))}
              ...
              <div className="checkout-section" style={{padding:20}}>
                ...
                {[
                  ...
                  { step:"4", text:`Submit — we verify and activate within ${settings.activationTime}` },
                ].map(s => (
              ...
              <div className="checkout-warning">
                🔒 Your payment is reviewed manually by our team. Plan activation typically takes 1–{settings.activationTime} after verification.
              </div>
            </div>
          </div>
        </div>
